...
Data's information in "${field}" should be equal "${value}"
    ...
    ${element}=               Get Element                   //label[contains(text(),"${field}")]/../../..//*[contains(@class,"ant-input")]
...
Table line should show empty 
  Wait Until Element Spin
  Get Property              //p[contains(@class, 'ant-empty-description')]                innerText                      equal 
...
Webpage should contains the "${name}" filter function
    ${element}=           Get Element                 //label*[contains(text(),"{$name}")]/../../nz-select
    ${count}=             Get Element Count             ${element}
    Should Be True    ${count} >= 1
...
Webpage should contain "${text}" input field
    ${element}=                Get Element                        //label[contains(text(),"${text}")]/../../..//input
    ${count}=                  Get Element Count                  ${element}
    Should Be True    ${count} >= 1
