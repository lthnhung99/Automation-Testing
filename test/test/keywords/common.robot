...
Wait Until Element Spin
    ...
    ${count}=    Get Element Count    ${element}
    IF    ${count} > 0    Wait Until Page Does Not Contain Element    ${element}
...
Click on eye icon in "${name}" field
    ${element}=    Get Element
    ...    //*[contains(@class, "ant-form-item-label")]/label[text()="${name}"]/../../*[contains(@class, "ant-form-item")]//input//ancestor::div[contains(@class, 'relative ng-star-inserted')]
...
Click on "${name}" check box
    ${element}=    Get Element
    ...    //span[contains(text(),"${name}")]//ancestor::label/span[contains(@class,'ant-checkbox')]
    Click    ${element}
...
"${name}" should not be visible in table line
    ...
    ELSE
        Get Text    //tbody/tr[2]    contain    No Data
    END
...
The hidden password in "${name}" field should be visibled as "${text}"
    ${text}=    Check Text    ${text}
    ${element}=    Get Element
    ...    //*[contains(@class, "ant-form-item-label")]/label[text()="${name}"]/../../*[contains(@class, "ant-form-item")]//input
...
Click filter "${name}" with "${text}"
    ...
    ${element}=    Get Element Form Item By Name
    ...    ${name}
    ...    //following-sibling::nz-select[contains(@class, "ant-select-show-arrow")]
    Click    ${element}
    Click    xpath=//*[contains(@class, "ant-select-item-option") and @title="${text}"]
    ${cnt}=    Get Length    ${text}
...
Webpage should contain left arrow icon
    ${element}=    Get Element    //i[contains(@class,'las la-arrow-left text-3xl text-blue-600')]
    ${count}=    Get Element Count    ${element}
    Should Be True    ${count} >= 1
...
Heading should contain "${text}" inner Text
    ...
    ${expected_text}=    Set Variable    ${expected_text.strip()}
    Get Text    //h2[contains(text(), "${text}")]    equal    ${expected_text}
...
Get the last account name
    ...
    IF    ${cnt} > 0    Set Global Variable    ${LAname}    ${LAname}
    RETURN    ${LAname}
...
Webpage should contain "${text}" input field
...
Webpage should contain "${text}" select field
    ${element}=    Get Element    //label[contains(text(), "${text}")]//ancestor::nz-form-item
    ${count}=    Get Element Count    ${element}
    Should Be True    ${count} >= 1
